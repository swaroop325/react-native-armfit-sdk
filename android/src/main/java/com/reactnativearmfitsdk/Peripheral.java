...
public class Peripheral extends BluetoothGattCallback {
	...
	private void sendConnectionEvent(BluetoothDevice device, String eventName, int status) {
		...
		Log.d(ArmfitSdkModule.LOG_TAG, "Peripheral event (" + eventName + "):" + device.getAddress());
	}
  ...
  public void connect(Callback callback, Activity activity) {
		if (!connected) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
				Log.d(ArmfitSdkModule.LOG_TAG, " Is Or Greater than M $mBluetoothDevice");
				gatt = device.connectGatt(activity, false, this, BluetoothDevice.TRANSPORT_LE);
			} else {
				Log.d(ArmfitSdkModule.LOG_TAG, " Less than M");
				try {
					Log.d(ArmfitSdkModule.LOG_TAG, " Trying TRANPORT LE with reflection");
		...
		if (gatt != null) {
			try {
				...
				if (force) {
					gatt.close();
					gatt = null;
					sendConnectionEvent(device, "ArmfitSdkModuleDisconnectPeripheral", BluetoothGatt.GATT_SUCCESS);
				}
				Log.d(ArmfitSdkModule.LOG_TAG, "Disconnect");
			} catch (Exception e) {
				sendConnectionEvent(device, "ArmfitSdkModuleDisconnectPeripheral", BluetoothGatt.GATT_FAILURE);
				Log.d(ArmfitSdkModule.LOG_TAG, "Error on disconnect", e);
			}
		} else
			Log.d(ArmfitSdkModule.LOG_TAG, "GATT is null");
	}
	...
	static WritableMap byteArrayToWritableMap(byte[] bytes) throws JSONException {
		...
		object.putArray("bytes", bytes != null ? ArmfitSdkModule.bytesToWritableArray(bytes) : null);
		return object;
	}
	...
	public void onConnectionStateChange(BluetoothGatt gatta, int status, int newState) {
		...
		Log.d(ArmfitSdkModule.LOG_TAG, "onConnectionStateChange to " + newState + " on peripheral: " + device.getAddress()
				+ " with status " + status);
		...
		if (newState == BluetoothProfile.STATE_CONNECTED) {
			...
			discoverServicesRunnable = new Runnable() {
				...
				public void run() {
					try {
						...
						Log.d(ArmfitSdkModule.LOG_TAG, "onConnectionStateChange connected but gatt of Run method was null");
					}
			...
			sendConnectionEvent(device, "ArmfitSdkModuleConnectPeripheral", status);

			if (connectCallback != null) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Connected to: " + device.getAddress());
	...
	public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
		...
		try {
			...
			if (buffer != null) {
				...
				// Log.d(ArmfitSdkModule.LOG_TAG, "onCharacteristicChanged-buffering: " +
				// buffer.size() + " from peripheral: " + device.getAddress());

				if (buffer.size().equals(buffer.maxCount)) {
					Log.d(ArmfitSdkModule.LOG_TAG, "onCharacteristicChanged sending buffered data " + buffer.size());
			...
			Log.d(ArmfitSdkModule.LOG_TAG, "onCharacteristicChanged: " + ArmfitSdkModule.bytesToHex(dataValue)
					+ " from peripheral: " + device.getAddress());
			WritableMap map = Arguments.createMap();
			map.putString("peripheral", device.getAddress());
			map.putString("characteristic", charString);
			map.putString("service", service);
			map.putArray("value", ArmfitSdkModule.bytesToWritableArray(dataValue));
			sendEvent("ArmfitSdkModuleDidUpdateValueForCharacteristic", map);

		} catch (Exception e) {
			Log.d(ArmfitSdkModule.LOG_TAG, "onCharacteristicChanged ERROR: " + e.toString());
		}
	}
	...
	public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
		...
		if (status != BluetoothGatt.GATT_SUCCESS) {
			if (status == GATT_AUTH_FAIL || status == GATT_INSUFFICIENT_AUTHENTICATION) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Read needs bonding");
			...
			mainHandler.post(new Runnable() {
					@Override
					public void run() {
						callback.invoke(null, ArmfitSdkModule.bytesToWritableArray(dataValue));
					}
				});
		}
	...
	public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
		...
		if (writeQueue.size() > 0) {
			...
			if (status == GATT_AUTH_FAIL || status == GATT_INSUFFICIENT_AUTHENTICATION) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Write needs bonding");
				// *not* doing completedCommand()
				return;
			}
	...
	public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
		if (registerNotifyCallback != null) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				Log.d(ArmfitSdkModule.LOG_TAG, "onDescriptorWrite success");
			} else {
				registerNotifyCallback.invoke("Error writing descriptor stats=" + status, null);
				Log.e(ArmfitSdkModule.LOG_TAG, "Error writing descriptor stats=" + status);
			}

			registerNotifyCallback = null;
		} else {
			Log.e(ArmfitSdkModule.LOG_TAG, "onDescriptorWrite with no callback");
		}
	...
	private void setNotify(UUID serviceUUID, UUID characteristicUUID, final Boolean notify, Callback callback) {
		...
		if ((characteristic.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
			Log.d(ArmfitSdkModule.LOG_TAG, "Characteristic " + characteristicUUID + " set NOTIFY");
			...
			Log.d(ArmfitSdkModule.LOG_TAG, "Characteristic " + characteristicUUID + " set INDICATE");
			...
			Log.d(ArmfitSdkModule.LOG_TAG, msg);
			callback.invoke(msg);
			return;
		}
		...
		boolean result = commandQueue.add(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						Log.d(ArmfitSdkModule.LOG_TAG, "Exception in setNotify", e);
					}
		...
		if (result) {
			...
			Log.e(ArmfitSdkModule.LOG_TAG, "Could not enqueue setNotify command");
		}
	}

	public void registerNotify(UUID serviceUUID, UUID characteristicUUID, Integer buffer, Callback callback) {
		Log.d(ArmfitSdkModule.LOG_TAG, "registerNotify");
	...
	public void removeNotify(UUID serviceUUID, UUID characteristicUUID, Callback callback) {
		Log.d(ArmfitSdkModule.LOG_TAG, "removeNotify");
	...
	private BluetoothGattCharacteristic findNotifyCharacteristic(BluetoothGattService service,
		...
		try {
			...
			Log.e(ArmfitSdkModule.LOG_TAG, "Error retriving characteristic " + characteristicUUID, e);
			return null;
		}
	}
	...
	public void read(UUID serviceUUID, UUID characteristicUUID, Callback callback) {
		...
		if (result) {
			...
			Log.d(ArmfitSdkModule.LOG_TAG, "Could not queue read characteristic command");
		}
	}
	...
	private void nextCommand() {
		synchronized (this) {
			if (commandQueueBusy) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Command queue busy");
				return;
			}

			final Runnable nextCommand = commandQueue.peek();
			if (nextCommand == null) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Command queue empty");
				return;
			}

			// Check if we still have a valid gatt object
			if (gatt == null) {
				Log.d(ArmfitSdkModule.LOG_TAG, "Error, gatt is null");
				commandQueue.clear();
				commandQueueBusy = false;
				return;
			}
			...
			mainHandler.post(new Runnable() {
					...
					public void run() {
						try {
							...
							Log.d(ArmfitSdkModule.LOG_TAG, "Error, command exception");
							completedCommand();
						}
					}
				});
	...
	public void readRSSI(final Callback callback) {
		...
		if (result) {
		    ...
		    Log.d(ArmfitSdkModule.LOG_TAG, "Could not queue readRemoteRssi command");
		}
	}
	...
	public boolean doWrite(final BluetoothGattCharacteristic characteristic, byte[] data, final Callback callback) {
		...
		if (result) {
			...
			Log.d(ArmfitSdkModule.LOG_TAG, "Could not queue write characteristic command");
		}
	...
	private BluetoothGattCharacteristic findWritableCharacteristic(BluetoothGattService service,
		...
		try {
			...
			Log.e(ArmfitSdkModule.LOG_TAG, "Error on findWritableCharacteristic", e);
			return null;
		}
	}
